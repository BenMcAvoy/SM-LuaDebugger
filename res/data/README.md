...
- Drops `emmy_core.dll` (embedded in the binary) to a temp file with `FILE_FLAG_DELETE_ON_CLOSE`, `LoadLibrary`s it, and immediately closes the handle so the file is unlinked from disk while the loaded image stays mapped.
